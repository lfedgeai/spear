...
func (p *DockerTask) Start() error {
	err := p.runtime.cli.ContainerStart(context.TODO(), p.container.ID,
	...
	go func() {
		for {
			...
			for {
				...
				n = n - 8
				// big endian size
				sz := int(header[4])<<24 | int(header[5])<<16 |
					int(header[6])<<8 | int(header[7])
...
func (p *DockerTask) Wait() (int, error) {
	c, err := p.runtime.cli.ContainerWait(context.TODO(), p.container.ID,
